...
\phantom
\phantom
\noindent From the method outlined in \emph{Chapter 5}, the following statistics were derived for the study area (i.e., fires in ecoregions 3E, 3W and 3S, over the period 1989--2004).
...
<<echo=FALSE,results=tex>>=
      library(xtable)
      Nt.table <- xtable(Nt,align="|l|r|r|",caption="The number of forest fires--per year (Nt)", label="tab3")
      print(Nt.table, include.rownames=FALSE)
@
...
<<echo=FALSE,results=tex>>=
      library(xtable)
      At.table <- xtable(At,align="|l|r|r|",caption="The area burned by forest fires, per year (At)", label="tab4")
      digits(At.table) <- 0
      print(At.table, include.rownames=FALSE)
@
...
\begin{figure}[h!]
  \centering
    \includegraphics[width=1\textwidth]{media/fig9}
      \caption[Mean hectares burned by size class]{Mean hectares burned by size class (1989--2004) in the Intensive and Measured fire management zones.}
        \label{fig9}
\end{figure}

\noindent These results are consistent with Cumming's (2005:2) observation (see \emph{Chapter 3.1}) that fires greater that 200 ha, account for the vast majority  of the area burned by forest fires every year (98\% in this case). As such, preventing fires from becoming this large is a primary concern for fire managers in Ontario.
...
\begin{figure}[h!]
  \centering
    \includegraphics[width=.8\textwidth]{media/fig10}
      \caption[Percentage of fires by size class]{Percentage of fires by size class (1989--2004) in the Intensive and Measured fire management zones.}
        \label{fig10}
\end{figure}
...
\begin{figure}[h!]
  \centering
    \includegraphics[width=.8\textwidth]{media/fig11}
      \caption[Percentage of fires for size classes 40 ha to 10000+ ha]{Percentage of fires for size classes 40 ha to 10000+ ha (1989--2004) in the Intensive and Measured fire management zones.}
        \label{fig11}
\end{figure}

\noindent From the analysis presented above, one might be tempted to draw the same conclusion as Ward and Tithecott (1993), that aggressive fire suppression in the Intensive zone has lead to far fewer moderate-- and large--sized fires. However, this would be based on the counter factual-- that without fire suppression the fire size distribution in the Intensive zone would have been the same as that of the Measured zone. Therefore, while these results are certainly indicative of the impact of fire suppression, it can not alone, prove the 'effectiveness' of fire suppression in Ontario.

\subsection{The Generalised Liner Model}

To measure the 'effectiveness' of fire suppression, a generalised liner model (GLM) was developed (see \emph{Chapter 5}), that would not only estimate the suppression probabilities between the fire management zone, but also control for other environmental factors that may be influencing the distribution of forest fires in Ontario. \\
...
out <- glm(glm.eq, family=binomial(logit), data=omnr.glm)

out.table <- xtable(out,align="r|rrrr",caption="Coefficients for the Generalised Liner Model (zone + ecoregion)",label="tab5")
...
<<echo=FALSE,results=tex>>=
      library(xtable)
      anova.table <- xtable(anova(out, test="Chisq"),align="r|rrrrr",caption="Analysis of variance for the Generalised Liner Model (zone + ecoregion)",label="tab6")
      print(anova.table)
@
...
The Akaike Information Criterion (AIC) was developed by Hirotsugu Akaike (1974), and is used to test a goodness of fit of a statistical model. However, the AIC also takes into account the tradeoff between the accuracy and complexity of the model. The AIC does this by penalising the model for increasing the number of interaction terms, thereby, discouraging over--fitting (i.e., adding spurious covariates simply to increase the goodness of fit). However, the AIC value only provides a relative measure of fit, and can therefore, only be used as a method for choosing between competing models (rather than hypothesis testing). \\
...
\noindent As such, model $SE$ (i.e., the full model used in the GLM above) best fits the data.
...
out.table <- xtable(out,align="r|rrrr",caption="Coefficients for the Generalised Liner Model (zone + load)",label="tab7")
...
<<echo=FALSE,results=tex>>=
      library(xtable)
      anova.table <- xtable(anova(out, test="Chisq"),align="r|rrrrr",caption="Analysis of variance for the Generalised Liner Model (zone + load)",label="tab8")
      print(anova.table)
@
